...
def run_akasa_scraper():
    ...
    # Initialize Queue: Each item is (route_string, retry_count)
    route_queue = [(r, 0) for r in top_20_routes]
    MAX_ROUTE_RETRIES = 1

    print("Launching Akasa Air Scraper (Fallback Queue Mode)...")

    with Stealth().use_sync(sync_playwright()) as p:
        browser = p.chromium.launch(headless=False, args=["--disable-blink-features=AutomationControlled"])

        while route_queue:
            route, retry_count = route_queue.pop(0)
            ...
            for window in advance_windows:
                ...
                print(f"\n--- Akasa Scraping: {route} | T+{window} Days (Queue Retry: {retry_count}) ---")
                ...
                # If both attempts for this window failed, break out and queue the route
                if not window_success:
                    print(f"❌ Route {route} completely failed on T+{window}.")
                    route_failed_midway = True
                    break
                    
            if route_failed_midway:
                if retry_count < MAX_ROUTE_RETRIES:
                    print(f"🔄 Pushing {route} to the back of the queue for a later retry.")
                    route_queue.append((route, retry_count + 1))
                else:
                    print(f"🚫 {route} exhausted all retries. Abandoning route.")
